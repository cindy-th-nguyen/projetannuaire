...
class FrontController extends AbstractController
{
    ...
    public function form_thematique()
    {
        ...
        return $this->render('thematique/form_thematique.html.twig', ['thematiques' => $thematiques]);
    }
     ...
     * @return mixed
     */
    public function import(Request $request, UserPasswordEncoderInterface $encoder)
    {
        $n=0;
               ...
               $path = $filename->getPathName();
                ...
                while (($column = fgetcsv($file, 1024, ",")) !== FALSE) {
                    if ($n > 0)
                    {
                        ...
                        $encoded = $encoder->encodePassword($compte, $pass);
                        ...
                        $office = $em->getRepository(Office::class)->findOneBy(array('code' => $column[10]));
                        $countries = $em->getRepository(Pays::class)->findOneBy(array('label' => $column[4]));
                        $buildings = $this->getDoctrine()->getRepository('App:Building')->findOneBy(array('label' => $column[11]));
                        $tutelles = $em->getRepository(Tutelle::class)->findOneBy(array('alias' => $column[12]));
                        $personne
                            ->setFirstname($column[0])
                            ->setLastname($column[1])
                            ->setCivilite($civilites[$column[2]])
